...
@section('content')
    <div class="container"
        ...
        <form action="{{ route('admin.compound_amenities.save') }}" method="POST" enctype="multipart/form-data"
            class="invoice_form card p-3 m-2 shadow">
            @csrf
            <h1>{{ $compound->en_name }}</h1>
            <div class="row">
                <input type="hidden" name="compound_id" value="{{ $compound->id }}" readonly>
        ...
        <div class="table-responsive">
            ...
            <table class="table" id="prd_table">
                ...
                <tbody id="products_table_body">
                    ...
                    @foreach ($compound_amenities as $item)
                        <tr>
                            <td>{{ $item->amenity_name }} </td>
                            <td> {{ $item->compound_name }} </td>
                            <td><a class="btn btn-danger"
                                    href="{{ route('admin.compound_amenities.delete', ['amenity_id' => $item->amenity_id, 'compound_id' => $item->compound_id]) }}"><i
